/**
 * Logique de récupération des deals — utilisée directement par les pages serveur
 * et par le route handler /api/deals (sans HTTP auto-appel au build).
 *
 * Priorité :
 * 1. Ryanair public API (aucune clé)
 * 2. Kiwi.com (si KIWI_API_KEY dispo)
 * 3. Mock + liens Kayak (fallback)
 */
...
export function ago(minutes: number) { return new Date(Date.now() - minutes * 60_000).toISOString(); }
export function fromNow(hours: number) { return new Date(Date.now() + hours * 3_600_000).toISOString(); }
export function addDays(n: number) { const d = new Date(); d.setDate(d.getDate() + n); return d.toISOString().slice(0, 10); }

export function priceHistory(avg: number, currentPrice: number, days = 30) {
...
export const DEFAULT_AIRPORTS = ["BVA", "STN", "BCN", "DUB", "BGY"];

export async function getRyanairDeals(airports: string[]): Promise<GlitchDeal[]> {
  const allFares = await Promise.allSettled(airports.map(ap => getCheapFares(ap)));
  const deals: GlitchDeal[] = [];
  const now = new Date().toISOString();

  for (let i = 0; i < airports.length; i++) {
    const apCode = airports[i];
    const result = allFares[i];
    if (result.status !== "fulfilled") continue;

    const sorted = [...result.value].sort((a, b) => a.outbound.price.value - b.outbound.price.value);
    ...
    for (const fare of sorted) {
      ...
      const depAirport = AIRPORTS_BY_CODE[apCode];
      const depCity    = depAirport?.city ?? apCode;
      ...
      deals.push({
        ...
        currency:     "EUR",
        departDate,
        bookingUrl:   ryanairBookingUrl(fromCode, toCode, departDate),
        source:       "Ryanair.com",
        detectedAt:   now,
...
export async function getKiwiDeals(): Promise<GlitchDeal[]> {
  const now = new Date().toISOString();
...
export function getMockDeals(): GlitchDeal[] {
  const raw = [
    ...
    { from: "Londres", fromCode: "LHR", to: "Sydney",      toCode: "SYD", flag: "🇦🇺", airline: "Qantas",         airlineCode: "QF", cabin: "economy"  as const, currentPrice: 290, normalPrice: 980,  avg7d: 950,  lowestEver: 540,  departDate: addDays(162), returnDate: undefined,    detectedAt: ago(15), expiresEstimate: fromNow(3), stops: 1, confirmations: 62, reports: 4, comments: 35 },
  ...
  return raw.map((r, i) => {
    ...
    return {
      ...
      bookingUrl: kayakLink({ from: r.fromCode, to: r.toCode, departDate: r.departDate, returnDate: r.returnDate, cabin: r.cabin as any }),
...
/**
 * Récupère les deals depuis toutes les sources disponibles.
 * Peut être appelé directement depuis les pages serveur sans HTTP.
 */
export async function fetchDeals(airports: string[] = DEFAULT_AIRPORTS): Promise<GlitchDeal[]> {
  const hasKiwiKey = !!process.env.KIWI_API_KEY;

  const [ryanairDeals, kiwiDeals] = await Promise.all([
    getRyanairDeals(airports).catch(() => [] as GlitchDeal[]),
  ...
  return deals.sort((a, b) => b.analysis.confidence - a.analysis.confidence);
}
