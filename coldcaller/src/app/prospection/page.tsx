...
import {
  ...
  Building2, MapPin, Phone, Globe, CheckCircle, AlertCircle,
  RefreshCw, Sparkles, X, ExternalLink, Copy, Star,
  TrendingUp, Target, PhoneCall, PhoneMissed, PhoneOff,
  MessageSquare, Hash, Calendar, Briefcase, ChevronDown,
  CheckSquare, Database, Clock, RotateCcw, ChevronRight,
} from "lucide-react";
...
function EmailSourceIcon({ source }: { source?: string }) {
  ...
  if (source === "pattern_email")    return <span title="Email pattern — à vérifier"><AlertCircle className="w-3 h-3 text-amber-400 shrink-0" /></span>;
...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button onClick={copy} className="p-1 rounded text-white/20 hover:text-white/60 transition-colors" title="Copier">
...
export default function ProspectionPage() {
  ...
  // ── Notes locales (éditées dans le panneau) ──────────────────────────────
  ...
  // ── Mise à jour optimiste d'un prospect ──────────────────────────────────
  ...
  const dansCrm     = prospects.filter((p) => p.danscrm).length;
          ...
          <div className="shrink-0 px-6 pt-5 pb-3 border-b border-white/[0.06]">
            <div className="flex items-center justify-between mb-3">
              <div>
                ...
                <p className="text-xs text-white/30 mt-0.5">SIRENE → enrichissement → email IA → suivi commercial</p>
              </div>
              ...
              <div className="glass rounded-xl p-4 space-y-3">
                <div className="grid grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    <input value={nafCustom} onChange={(e) => setNafCustom(e.target.value)}
                      ...
                      className="input w-full text-xs placeholder:text-white/20" />
                  </div>
                  {/* Département */}
                  <div>
                  ...
                  <div>
                    ...
                    <select value={tranche} onChange={(e) => setTranche(e.target.value)} className="select w-full text-xs">
                      ...
                      {Object.entries(TRANCHES_EFFECTIFS).map(([k, v]) => (
                        k !== "NN" && <option key={k} value={k}>{v}</option>
                      ))}
                    </select>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <button onClick={handleSearch} disabled={loading}
                    ...
                    {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Recherche SIRENE…</> : <><Zap className="w-4 h-4" /> Trouver des prospects</>}
                  </button>
            ...
            <div className="shrink-0 px-6 py-3 border-b border-white/[0.06] space-y-2">
              ...
              {enrichProgress && (
                <div className="flex items-center gap-3 glass rounded-lg px-3 py-2">
                  ...
                  <button onClick={() => setEnrichProgress(null)} className="text-white/20 hover:text-white/50">
                    <X className="w-3 h-3" />
                  </button>
                </div>
              )}
                  ...
                  {prospects.map((p) => {
                    ...
                    return (
                      <tr key={p.id} onClick={() => setSelected(p)}
                        ...
                        <td className="px-4 py-2.5 whitespace-nowrap">
                          {p.telephonePro
                            ? <a href={`tel:${p.telephonePro.replace(/\s/g,"")}`} onClick={(e)=>e.stopPropagation()}
                                className="font-mono text-brand-300 hover:text-brand-200 text-[10px] transition-colors">
                                {p.telephonePro}
                              </a>
                        ...
                        {/* Statut appel */}
                        <td className="px-4 py-2.5 whitespace-nowrap">
                          <AppelBadge statut={p.statutAppel} />
                        </td>
                        {/* Pipeline */}
                        <td className="px-4 py-2.5 whitespace-nowrap">
          ...
          <div className="w-[460px] shrink-0 border-l border-white/[0.06] flex flex-col bg-ink-950/60 overflow-hidden">
            ...
            <div className="shrink-0 px-5 py-4 border-b border-white/[0.06] bg-ink-950/95 backdrop-blur">
              <div className="flex items-start justify-between gap-3">
                ...
                <button onClick={() => setSelected(null)} className="text-white/30 hover:text-white transition-colors shrink-0 mt-0.5">
                  <X className="w-4 h-4" />
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              {/* ── Actions rapides ── */}
              <div className="px-5 py-3 border-b border-white/[0.04] flex gap-2 flex-wrap">
                ...
                <button onClick={() => generateEmail(selected)} disabled={generating.has(selected.id)}
                  className="flex items-center gap-1.5 bg-violet-500/15 hover:bg-violet-500/25 text-violet-300 border border-violet-500/30 rounded-lg px-3 py-1.5 text-xs font-medium transition-all disabled:opacity-50">
              ...
              <div className="px-5 py-4 border-b border-white/[0.06]">
                    ...
                    return (
                      <button key={k}
                        ...
                        className={cn(
                          ...
                          active
                            ? STATUT_APPEL_COLORS[k] + " shadow-sm"
                ...
                {/* CRM */}
                <button
                  onClick={() => updateProspect(selected.id, { danscrm: !selected.danscrm, dateCrm: !selected.danscrm ? new Date().toISOString() : undefined })}
                  className={cn(
                    "flex items-center gap-2 w-full px-3 py-2.5 rounded-lg border text-xs font-medium transition-all mb-3",
                    selected.danscrm
                      ? "bg-green-500/15 border-green-500/30 text-green-300"
                      : "bg-white/[0.03] border-white/[0.06] text-white/40 hover:text-white/70 hover:bg-white/[0.06]"
                  )}>
                  <Database className={cn("w-4 h-4", selected.danscrm ? "text-green-400" : "")} />
                  {selected.danscrm ? "Dans le CRM ✓" : "Ajouter au CRM"}
                  {selected.dateCrm && <span className="ml-auto text-[10px] text-green-400/60">
                    {new Date(selected.dateCrm).toLocaleDateString("fr-FR")}
                  </span>}
                </button>
                ...
                <textarea
                  ...
                  onBlur={saveNotes}
                  placeholder="Observations, prochaine action, contexte de l'appel…"
                  rows={3}
                ...
                />
                {notesDirty && (
                  <button onClick={saveNotes} className="mt-1.5 text-[10px] text-brand-400 hover:text-brand-300 transition-colors">
              ...
              <div className="px-5 py-4 border-b border-white/[0.06]">
                ...
                <div className="space-y-2">
                  <Row icon={Hash} label="SIREN" value={selected.siren} copy />
                  {selected.siret && <Row icon={Hash} label="SIRET (siège)" value={selected.siret} copy />}
                  <Row icon={Briefcase} label="Activité" value={selected.libelleNaf ? `${selected.libelleNaf} (${selected.codeNaf})` : selected.codeNaf} />
                  {selected.adresse && <Row icon={MapPin} label="Adresse" value={[selected.adresse, selected.codePostal, selected.ville].filter(Boolean).join(", ")} />}
                  {selected.trancheEffectifs && (
                    <Row icon={Users} label="Effectifs" value={TRANCHES_EFFECTIFS[selected.trancheEffectifs] ?? selected.trancheEffectifs} />
                  )}
                  {selected.dateCreation && (
                    <Row icon={Calendar} label="Création" value={selected.dateCreation.slice(0,4)} />
                  )}
                </div>
              </div>
              ...
              <div className="px-5 py-4 border-b border-white/[0.06]">
                ...
                {selected.dirigeantPrincipal ? (
                    ...
                    {selected.dirigeants.length > 1 && (
                      <details className="mt-2">
                        <summary className="text-[10px] text-white/30 cursor-pointer hover:text-white/50">
                          {selected.dirigeants.length} dirigeants au total
                        </summary>
                  ...
                  <p className="text-xs text-white/30 italic">Non identifié</p>
                )}
              </div>

              {/* ── Contact ── */}
              <div className="px-5 py-4 border-b border-white/[0.06]">
                <p className="text-[10px] text-white/30 font-semibold uppercase tracking-wider mb-3">Contact</p>
                <div className="space-y-2">
                  {/* Email */}
                  {selected.emailDirigeant ? (
                    <div className="flex items-center gap-2 bg-white/[0.04] rounded-lg px-3 py-2">
                      <div className="flex items-center gap-1.5 flex-1 min-w-0">
                        ...
                        <span className="font-mono text-violet-300 text-xs truncate">{selected.emailDirigeant}</span>
                      </div>
                  ...
                  <div className="space-y-2.5">
                    {selected.problematique && (
                      <div>
                        ...
                        <p className="text-xs text-white/70 leading-relaxed">{selected.problematique}</p>
                      </div>
                    )}
                    {selected.observation && (
                      <div>
                        <p className="text-[10px] text-white/30 mb-1">Observation spécifique</p>
                        <p className="text-xs text-white/70 leading-relaxed">{selected.observation}</p>
                      </div>
                    )}
                    {selected.angleApproche && (
                      <div>
                        <p className="text-[10px] text-white/30 mb-1">Angle d'approche</p>
                        <p className="text-xs text-white/70 leading-relaxed">{selected.angleApproche}</p>
                      </div>
                    )}
                  </div>
                </div>
...
function Row({ icon: Icon, label, value, copy }: {
